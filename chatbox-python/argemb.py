...
def extract_agent_arguments(conversation):
    ...
    for idx in range(1, len(filtered_arguments), 2):
            agent_text = filtered_arguments[idx]
            agent_index = transcript.find(agent_text)
    if agent_index != -1:
        agnet_embedding = embedding[agent_index:agent_index + len(agent_text)]
        agent_embeddings.append(agent_embedding)
    else:
        print(f"agnet statement not found in transcript: {agent_text}")
...
for idx, agent_embedding in enumerate(agent_embeddings):
    print(f"agent {idx+1} embedding: {agent_embedding}")